...
const DEFAULT_TRACE_DIR = path.join(".tmp", "catalog-benchmark", "device-traces");
const DEFAULT_SUMMARY_PATH = path.join(".tmp", "catalog-benchmark", "results", "device-summary.json");
...
export function validateDeviceTraces(traceDir = DEFAULT_TRACE_DIR): {
	...
	if (seeds.size !== 1 || [...seeds][0] !== "knomo-catalog-30k-v1") throw new Error("Device traces must use the frozen PERF-30K fixture.");
