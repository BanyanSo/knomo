...
// 编辑器高度来自移动端可用空间测量；仅允许这一动态约束写入。
// 路径、完整语句及单次出现均须匹配，不能豁免整个文件或同一行的其他操作。
const ALLOWED_SOURCE_STATEMENTS = new Map([
...
function collectMatches(targetPath, pattern, matches) {
	...
	const allowedStatement = pattern === FORBIDDEN_SOURCE_PATTERN
		? ALLOWED_SOURCE_STATEMENTS.get(toPosix(path.relative(process.cwd(), path.resolve(targetPath))))
		: undefined;
	const hasSingleAllowedStatement = allowedStatement !== undefined
		&& lines.filter((line) => line.trim() === allowedStatement).length === 1;
	for (let index = 0; index < lines.length; index += 1) {
		const line = lines[index];
		if (hasSingleAllowedStatement && line.trim() === allowedStatement) continue;
