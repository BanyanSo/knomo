...
import { canonicalJson, sha256Bytes, sha256Text } from "./CatalogV2Protocol";

export const CATALOG_V2_MONTHLY_READONLY_COMMENT = [
	"<!-- knomo:monthly-archive",
	translate("zh-CN", "archive.deterministicReadOnlyComment"),
	"-->",
].join("\n");
export const CATALOG_V2_MONTHLY_RENDERER_VERSION = 1;
...
export interface CatalogV2MonthlyProjectionInput {
	period: string;
...
export interface CatalogV2MonthlyProjectionResult {
	period: string;
	path: string;
	content: string;
	bytes: Uint8Array;
...
export async function buildCatalogV2MonthlyProjection(
	input: CatalogV2MonthlyProjectionInput,
	markdownBlockService = new MarkdownBlockService(),
): Promise<CatalogV2MonthlyProjectionResult> {
	...
	const sections = dates.map((logicalDate) => {
		const heading = formatCatalogV2MonthlyDateHeading(input.settings.monthlyDateHeadingFormat, logicalDate);
	...
	const rendererVersion = input.rendererVersion ?? CATALOG_V2_MONTHLY_RENDERER_VERSION;
	const semanticValue = {
		period: input.period,
		rendererVersion,
		targetPath: getCatalogV2MonthlyArchivePath(input.settings, input.period),
	...
	return {
		period: input.period,
		path: getCatalogV2MonthlyArchivePath(input.settings, input.period),
		content,
		bytes,
		rendererVersion,
		sourceDigest,
...
export function getCatalogV2MonthlyArchivePath(
...
export function getCatalogV2MonthlyConflictPeriod(
	settings: Pick<KnomoSettings, "monthlyMemoFolder" | "monthlyMemoFileFormat">,
	path: string,
): string | null {
	...
	const canonicalPath = getCatalogV2MonthlyArchivePath(settings, period);
...
export function getCatalogV2MonthlyCanonicalPeriod(
	settings: Pick<KnomoSettings, "monthlyMemoFolder" | "monthlyMemoFileFormat">,
	path: string,
): string | null {
	const normalizedPath = normalizePath(path);
	const period = normalizedPath.match(/\d{4}-(?:0[1-9]|1[0-2])/)?.[0] ?? null;
	return period !== null && getCatalogV2MonthlyArchivePath(settings, period) === normalizedPath ? period : null;
}

export function formatCatalogV2MonthlyDateHeading(format: string, logicalDate: string): string {
