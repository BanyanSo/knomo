...
import {
	...
	getCatalogV2MonthlyArchivePath,
} from "./CatalogV2MonthlyProjection";
import type { CatalogV2ProjectionInputBuilder } from "./CatalogV2ProjectionInputBuilder";
...
export interface CatalogV2MonthlyProjectionMetadata {
	rendererVersion: number;
	sourceDigest: string;
	outputHash: string;
}

export interface CatalogV2MonthlyProjectionCoordinatorOptions {
	inputBuilder: CatalogV2ProjectionInputBuilder;
...
export class CatalogV2MonthlyProjectionCoordinator {
	...
	private readonly metadata = new Map<string, CatalogV2MonthlyProjectionMetadata>();
	...
	constructor(
		...
		private readonly options: CatalogV2MonthlyProjectionCoordinatorOptions,
	) {
	...
	private async project(period: string): Promise<void> {
		...
		const projection = await buildCatalogV2MonthlyProjection({
			period,
...
function toMetadata(projection: Awaited<ReturnType<typeof buildCatalogV2MonthlyProjection>>): CatalogV2MonthlyProjectionMetadata {
	return {
...
export function getMonthlyProjectionTargetPath(settings: KnomoSettings, period: string): string {
	return getCatalogV2MonthlyArchivePath(settings, period);
}
