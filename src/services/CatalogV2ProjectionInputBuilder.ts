...
import {
	...
	getCatalogV2MonthlyCanonicalPeriod,
...
export interface CatalogV2ProjectionBuildResult {
	period: string;
...
export interface CatalogV2ProjectionInputBuilderOptions {
...
export class CatalogV2ProjectionInputBuilder {
	private readonly getRendererVersion: () => number;

	constructor(
		private readonly app: App,
		private readonly parser: DiaryMemoParser,
		private readonly options: CatalogV2ProjectionInputBuilderOptions,
	) {
		this.getRendererVersion = options.getRendererVersion ?? (() => CATALOG_V2_MONTHLY_RENDERER_VERSION);
	}
	...
	async listPeriods(): Promise<string[]> {
		...
		for (const file of this.app.vault.getMarkdownFiles()) {
			...
			const monthlyPeriod = getCatalogV2MonthlyCanonicalPeriod(settings, file.path);
			if (monthlyPeriod !== null) periods.add(monthlyPeriod);
		}
		return [...periods].sort();
	}

	async build(period: string): Promise<CatalogV2ProjectionBuildResult> {
		assertPeriod(period);
		const dailyConfig = await this.options.getDailyConfig();
		...
		const sourceDigest = await sha256Text(canonicalJson({
			...
			monthly: settings,
			targetPath: getCatalogV2MonthlyArchivePath(settings, period),
		}));
		return {
			period,
			observations,
	...
	getMonthlyPeriod(path: string): string | null {
		return getCatalogV2MonthlyCanonicalPeriod(this.getSettings(), path);
	}
