...
export function calculateMobileComposerDockOffset(input: MobileComposerDockOffsetInput): number {
	const keyboardHeight = Math.max(0, input.baselineHeight - input.keyboardTop);
	const nextBottomOffset = keyboardHeight + input.targetGap - input.toolbarAnchorInset;
	return Math.round(Math.max(0, nextBottomOffset));
}
