...
import { CatalogV2DailyWriteGateway, CatalogV2StaleDailyError } from "../src/services/CatalogV2DailyWriteGateway";
...
test("expected revision and late editor changes reject stale Daily writes", async () => {
	...
	await assert.rejects(() => gateway.commit(prepared), CatalogV2StaleDailyError);
	assert.equal(editor.transactionCount, 0);
});

function makeGateway(app: App): CatalogV2DailyWriteGateway {
	return new CatalogV2DailyWriteGateway(app, new DiaryMemoParser(async (bytes) => (
