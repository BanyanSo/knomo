...
import { CatalogV2MonthlyProjectionCoordinator } from "../src/services/CatalogV2MonthlyProjectionCoordinator";
import { CatalogV2ProjectionInputBuilder } from "../src/services/CatalogV2ProjectionInputBuilder";
...
function createFixture(
	...
	const inputBuilder = new CatalogV2ProjectionInputBuilder(replica.app, parser, {
...
async function invokeChanged(coordinator: CatalogV2MonthlyProjectionCoordinator, file: TFile): Promise<void> {
	await (coordinator as unknown as { handleMonthlyFileChanged(value: unknown): Promise<void> })
		.handleMonthlyFileChanged(file);
}

async function invokeVaultChanged(coordinator: CatalogV2MonthlyProjectionCoordinator, file: TFile): Promise<void> {
	await (coordinator as unknown as { handleFileChanged(value: unknown): Promise<void> })
		.handleFileChanged(file);
}

async function invokeDeleted(coordinator: CatalogV2MonthlyProjectionCoordinator, file: TFile): Promise<void> {
