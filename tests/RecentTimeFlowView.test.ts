...
test("午夜无 Memo 变化仍刷新，恢复时同日期也重排计时，关闭后不回调", async () => {
	...
	assert.deepEqual(calls, ["stop", "start", "ui", "refresh"]);
	calls.length = 0;
	view.handleLocalDateChange();
...
test("真实 revision key 的普通与浮标两种到达顺序收敛，同文 occurrence 不丢失", async () => {
	...
	for (const buoyFirst of [false, true]) {
		...
		revision = 2;
		promoted = [memo("b", 1)];
		view.catalogTodayTimeBuoys = controller.getSnapshot();
		if (buoyFirst) view.catalogTodayTimeBuoys = await controller.prepareTodayOnly();
		else view.catalogRevision = 2;
		assert.deepEqual(view.getTodayTimeBuoyItems(), []);
		if (buoyFirst) view.catalogRevision = 2;
		else view.catalogTodayTimeBuoys = await controller.prepareTodayOnly();
		...
		promoted = [];
		view.catalogTodayTimeBuoys = await controller.prepareTodayOnly();
		assert.deepEqual(view.getTodayTimeBuoyItems(), []);
