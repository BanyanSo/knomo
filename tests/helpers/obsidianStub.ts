...
function buildObsidianStub(): string {
	return [
		...
		"};",
		"let localeValue = 'en';",
		"const momentDate = (date, initialLocale = localeValue) => {",
		"  let dateLocale = initialLocale;",
		"  const value = {",
		"  isValid: () => !Number.isNaN(date.getTime()),",
		"  toDate: () => new Date(date),",
		"  month: () => date.getMonth(),",
		"  locale: (nextLocale) => { if (typeof nextLocale !== 'string') return dateLocale; dateLocale = nextLocale; return value; },",
		"  format: (format = 'YYYY-MM-DD') => String(format).replace(/YYYY|MMMM|dddd|MM|DD|HH|mm|ss|M|D/g, (token) => {",
		"    const months = localizedMonths[dateLocale] ?? localizedMonths.en;",
		"    const weekdays = localizedWeekdays[dateLocale] ?? localizedWeekdays.en;",
		"    const values = {",
		...
		"  const date = match === null ? new Date(input) : new Date(Date.UTC(Number(match[1]), Number(match[2]) - 1, Number(match[3])));",
		"  return momentDate(date);",
		"};",
		...
		"  addChild(child) { this.children.push(child); child.load?.(); return child; }",
		"  removeChild(child) { this.children = this.children.filter((item) => item !== child); child.unload?.(); }",
		"  load() { this.onload?.(); }",
		"  unload() { for (const child of [...this.children]) child.unload?.(); for (const cleanup of this.cleanups.splice(0)) cleanup(); this.onunload?.(); }",
		"  register(cleanup) { this.cleanups.push(cleanup); }",
		"  registerEvent() {}",
		"  registerDomEvent(target, type, listener, options) { target?.addEventListener?.(type, listener, options); this.register(() => target?.removeEventListener?.(type, listener, options)); }",
		"}",
